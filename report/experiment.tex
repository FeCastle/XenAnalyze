...
We calculate the $Overhead_V$ in Dom1 using the same method as described previously. 

\begin{table}[h]
\begin{tabular}{ l l l p{5cm} }
  Counter     & Dom0 & Dom1 & $Overhead_V$ \\
  \hline
    \textbf{read/ms}    & 275  & 281 & \\
...
Now we see how adding a external guest domains effect the interference by adding 1 domain at a time and calculating the interference and the drop in application performance.
\begin{table}[!h]
\begin{tabular}{ l l l l l l p{9cm} }
                   & TPS   & reads/ms & reads/ms & AvgRdWait & $Interference_{RPS}$ & $Inteference_{AWR}$ \\

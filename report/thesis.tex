% \documentclass[10pt,twocolumn,oneside]{article}
\documentclass[12pt,onecolumn,oneside]{article}
% \usepackage{simpleConference}
% \usepackage{times}
\usepackage{mathptmx}
\usepackage{graphicx}
\usepackage{colortbl}
\usepackage{todonotes}
\usepackage{url,hyperref}
\hypersetup{colorlinks=true, linkcolor=blue, citecolor=black}
\usepackage{fancyvrb}
\usepackage{verbatim}
\usepackage[top=1in, bottom=1.0in, left=1.5in, right=1in, includefoot]{geometry}
\usepackage{fixltx2e}
% Install package: texlive-science
\usepackage{algorithmic} 
% singlelinecheck (false) - Remove some of the space between figure and caption
% skip = 0pt
% Install package: texlive-generic-extra
\usepackage[font=small,labelfont=bf, singlelinecheck=false, skip=0pt]{caption}
\usepackage{subcaption}
\usepackage{setspace}

% 12pt font for section headers
\usepackage{titlesec}
%\titleformat{\section}{\bfseries}{\thesection}{1em}{}
\titleformat*{\section}{\normalfont\normalsize\bfseries\color{black}}
\titleformat*{\subsection}{\normalfont\normalsize\bfseries\color{black}}
\titleformat*{\subsubsection}{\normalfont\normalsize\bfseries\color{black}}


%Remove hyphens
\usepackage[none]{hyphenat}

\begin{document}
% Turn of page numbering for now
\pagenumbering{gobble}

\begin{comment}
\title{System-wide Performance Analysis for Virtualization}
\author{Deron Jensen\\
\\
Department of Computer Science\\
Portland State University\\
Portland, OR \\
deron@cs.pdx.edu \\
}
\date{\today}
  \maketitle
\end{comment}

% Make my own title page liKe: http://www.pdx.edu/sites/www.pdx.edu.ogs/files/ETD.thesis.titlepg.example.pdf
\input{title}
\newpage

% Reset the geometry to include footer so that pages are added
% \newgeometry{top=1in, bottom=1.0in, left=1.5in, right=1in, includefoot}

% Force the margin requirements!!!
\sloppy

% prefatory pages - According to PSU reviewer abstract should be page (i)
\setcounter{page}{1}
\pagenumbering{roman}

\begin{abstract}
  \doublespacing
  \normalsize
  \input{abstract}
\end{abstract}

\singlespacing
\newpage
% \end{@twocolumnfalse}

\tableofcontents    % Table of Contents
\newpage
\listoftables
\newpage
\listoffigures 
\newpage

%definitions
\input{definitions.tex}
\newpage

\onecolumn

% TODO Set up Double Space
\doublespacing
\setcounter{page}{1}
\pagenumbering{arabic}
% 1.  This is a comment
\section{Introduction}
\input{introduction}
\newpage

% This is in Intro now 
%  \section{Motivating Example}
% \input{motivating}
% \newpage

% \section{Current Tools}
% \newpage

\section{Related Work}
\input{related}
\input{virtualization}
\newpage

\section{Methodology}
\input{implementation}
\subsection{Virtualization Test Suite}
\input{testSuite.tex}
\newpage

% \section{Test Suite for IO virtualization}
% \newpage

\section{Experimental Results}
\input{experiment.tex}
\newpage

\section{Discussion and Conclusions}
\input{discussion.tex}
\newpage

\input{references.tex}

\end{document}
